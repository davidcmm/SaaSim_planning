...
ggplot(dados, aes(x=risco, y=100 * ganho, ymin=100 * (ganho-ci), ymax=100 * (ganho+ci), colour=heur, fill=heur, linetype=heur)) + 
    geom_line(position=pd) + geom_ribbon(alpha=0.5)+
    geom_point(position=pd)+xlab("Risco de negação de serviço") +ylab("Ganho")+opts(title="Ganho de heurísticas em relação a heurística ON")+theme_bw()+scale_y_continuous(breaks=c(-4000, -3000, -2000, -1000, 0, 100, 200, 300, 400))
...
ggplot(dados, aes(x=risco, y=100 * ganho, ymin=100 * (ganho-ci), ymax=100 * (ganho+ci), colour=heur, fill=heur, linetype=heur)) + 
    geom_line(position=pd) + geom_ribbon(alpha=0.5) +
    geom_point(position=pd)+xlab("Risco de negação de serviço") +ylab("Ganho")+opts(title="Ganho de heurísticas em relação a heurística ON")+theme_bw()+scale_y_continuous(breaks=c(-70, -50, -30, -10, 0, 5, 10))
...
#    geom_point(position=pd)+xlab("Risco do Mercado Sob demanda") +ylab("Ganho")+opts(title="Ganho de heurísticas em relação a heurística ON")+theme_bw()
ggplot(dados, aes(x=risco, y=100 * ganho, ymin=100 * (ganho-ci), ymax=100 * (ganho+ci), colour=heur, fill=heur, linetype=heur)) + 
    geom_line(position=pd) + geom_ribbon(alpha=0.5)+ geom_point(position=pd)+xlab("Risco de negação de serviço") +ylab("Ganho")+opts(title="Ganho de heurísticas em relação a heurística ON")+theme_bw()+scale_y_continuous(breaks=c(-60, -50, -40, -30, -20, -10, 0, 1, 2, 4, 5))
