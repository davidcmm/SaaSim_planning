...
for value in `seq 1 18`; do #seq 1
	...
	if [ ${plan_heur} = "Optimal" ] ; then
		...
		rm .je.dat
		mv output.plan model.plan
		#cp ~/results_dissertacao/erro_m40/ut_100_5/model_${value}.plan ./model.plan
		echo "Planned"
	fi
	...
	#mv higher_utilization.dat result_${plan_heur}/higher_utilization_${value}       
        #mv lower_utilization.dat result_${plan_heur}/lower_utilization_${value}
	touch model.plan	
