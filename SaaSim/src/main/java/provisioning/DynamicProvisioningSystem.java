...
public class DynamicProvisioningSystem implements DPS{
	...
	/**
	 * This method is responsible to request a number of machines at IaaS providers and add
	 * them to load balancers.
	 * @param configurable
	 * @param tier
	...
	private void addServersToTier(DynamicConfigurable configurable, int tier, int numberOfInitialServers, List<MachineType> typeList) {
		...
		for (Provider provider : this.providers) {
			while(provider.canBuyMachine(false, MachineType.M1_SMALL) && serversAdded < numberOfInitialServers){
				configurable.addServer(tier, provider.buyMachine(false, MachineType.M1_SMALL), false);
				serversAdded++;
			}
			if(serversAdded == numberOfInitialServers){
				return;
	...
	/**
	 * Evaluates if a machine of a certain type can be purchased in the reservation or on-demand
	 * markets.
	 *  
	 * @param type
	 * @param isReserved Boolean indicating if the machine should be purchased in the reservation or
	 * on-demand market
	 * @return
	 */
