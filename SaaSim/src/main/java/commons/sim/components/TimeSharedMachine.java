...
/**
 * Machine that uses time sharing while executing requests.
 * 
...
public class TimeSharedMachine extends JEAbstractEventHandler implements Machine{
	...
	public void sendRequest(Request request) {
		...
		if(canRun()){//Request is processed by one of available threads
		...
		}else if(canQueue()){//Request waits in the backlog until a thread is available to process it
			this.backlog.add(request);
		}else{//Request is discarded!
	...
	/**
	 * Informs the load balancer that a request was processed and picks one request at
	...
	protected void scheduleNext() {
		...
		long nextQuantum = Math.min(nextRequest.getTotalToProcess(), 100);//Checkpointer.loadPriorities()[nextRequest.getSaasClient()]);
		lastUpdate = getScheduler().now();
	...
	/**
	 * This method estimates CPU utilization of current machine
	 * @param timeInMillis
	 * @return
	 */
	@Override
