...
public class JEEventScheduler implements Serializable{
     ...
     * @param simulationEnd Time at which simulation should end.
     * @throws IOException 
     */
	...
	private void removeUnnecessaryEvents() {
		Iterator<JEEvent> iterator = eventSet.iterator();
		while(iterator.hasNext()){
			JEEvent event = iterator.next();
			if(event.getType().equals(JEEventType.ESTIMATE_SERVERS)){
				iterator.remove();
			}
		}
	}
