...
public class SimpleSimulatorTest extends ValidConfigurationTest {
	...
	public void testRemoveServerWithoutDescriptorAndNotUseForce() {
		...
		EasyMock.expect(scheduler.registerHandler(EasyMock.anyObject(SimpleSimulator.class))).andReturn(1).times(2);
		EasyMock.expect(scheduler.now()).andReturn(0L).anyTimes();
		EasyMock.expect(handler.getHandlerId()).andReturn(1).anyTimes();
		
		EasyMock.expect(timeSharedMachine.getDescriptor()).andReturn(new MachineDescriptor(0, false, MachineType.C1_MEDIUM, 0)).anyTimes();
		EasyMock.expect(timeSharedMachine2.getDescriptor()).andReturn(new MachineDescriptor(1, false, MachineType.C1_MEDIUM, 0)).anyTimes();
		...
		assertTrue(loadBalancer.getServers().size() == 33);
		simulator.removeServer(0, false);
		assertTrue(loadBalancer.getServers().size() == 32);
	...
	public void testRemoveServerWithoutDescriptorAndUseForce() {
		...
		EasyMock.expect(timeSharedMachine.getDescriptor()).andReturn(new MachineDescriptor(0, true, MachineType.C1_MEDIUM, 0)).anyTimes();
		EasyMock.expect(timeSharedMachine2.getDescriptor()).andReturn(new MachineDescriptor(1, true, MachineType.C1_MEDIUM, 0)).anyTimes();
		Queue<Request> queue = new LinkedList<Request>();
	...
	@Test
	public void testStartWithoutBeingLastSimulationDayAndOptimalProvisioning() throws Exception{
		...
		Monitor monitor = EasyMock.createStrictMock(Monitor.class);
		EasyMock.expect(monitor.isOptimal()).andReturn(true);
		...
		assertEquals(JEEventType.READWORKLOAD, firstEvent.getValue().getType());
		assertEquals(JEEventType.ESTIMATE_SERVERS, secondEvent.getValue().getType());
	...
	@Test
	public void testStartInFirstChargeUsersDayWithOptimalProvisioning() throws Exception{
	...
	@Test
	public void testStartInOtherChargeUsersDayWithOptimalProvisioning() throws Exception{
